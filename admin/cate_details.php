...
$sql1 = "SELECT * from eventscat LIMIT $start, $limit";
                 ...
                 <aside class="profile-nav col-lg-12">   
                   <section class="panel" style="margin:0px auto; overflow:auto;">
                    <div class="panel-body">
                      <div class="row">
                      <div class="col-md-10">
                              <div class="input-group"><input id="show" placeholder="Search Here" onkeyup="getStates(this.value)" class="input-sm form-control" type="text">
                               <span class="input-group-btn">
                              <button type="button" class="btn btn-sm btn-success"> Go!</button> </span></div>
                          </div>
                          <div class="col-md-2">
              
                              <a href="cate_detailsadd.php"><button type="button" class="btn btn-sm btn-success" style="display:block; width:100%">Add</button></a> 
                           </div>
                      </div>
                    </div>
                      ...
                      <div class="table-responsive" id="demo" style="overflow: hidden; padding:20px 10px 0px 10px;" tabindex="1">
                        ...
                        <table class="table table-bordered" id="hide">
                         ...
                         <!--      <td class="p-name" style="text-align:center;">< ?php echo $row['name']; ?></td>
                              ...
                              {
                              ?>
                              <a href="cate_detailsactive.php?rid=<?php echo $row['id']; ?>" class="btn btn-sm btn-success" onclick="return confirm('Activate')"><i class=" fa fa-eye"></i></a>
                              <?php
                              }
                              if(($status)=='1')
                              {
                              ?>
                              <a href="cate_detailsactive.php?rid=<?php echo $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('De-activate')"><i class="fa fa-eye" aria-hidden="true"></i></a>
                              <?php
                              }
                              ?>
                              </td>
                              ...
                              <td class="p-progress" style="text-align:center;">  
                              <a href="cate_detailsrep.php?rep=<?php echo $row['id']; ?>"><button type="button" class="btn btn-sm btn-success"><i class="fa fa-pencil"></i></button></a>
                              </td>
                              <td class="p-progress" style="text-align:center;"> 
                              <a href="cate_detailsdel.php?del=<?php echo $row['id']; ?>"><button type="button" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure Delete this client')"><i class="fa fa-trash-o "></i></button></a></td>
                              </td>
...
<?php if($id>1) { ?>
<a href="cate_details.php?id=<?php echo ($id-1) ; ?>" class="btn btn-sm btn-success" >Privious</a>
<?php }

for ($i=1; $i<=$page; $i++)
{ ?>
<a href="cate_details.php?id=<?php echo $i; ?>" class="btn btn-sm btn-success" ><?php echo $i; ?></a>

<?php } ?>
<?php if($id!=$page) { ?>
<a href="cate_details.php?id=<?php echo ($id+1); ?>" class="btn btn-sm btn-success" >Next</a>
<?php } ?>
</div>
</div>  
      </div><br><br> <br><br> 
...
function getStates(value){
  $.post("cate_detailssearch.php",{partialState:value}, function(data){
    $("#demo").html(data);
  });
...
sliderdel("eventscat", $_POST['name']);
}  
...
slideractive("eventscat", $_POST['name']);
}
}

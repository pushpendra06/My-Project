...
function eventscategory(){
	include ('db.php');
	  $sql5 = "SELECT * from eventscat ";
	  $run5 = mysqli_query($conn, $sql5);
	  while($row5 = mysqli_fetch_array($run5)){
      ?>
       <li data-filter=".<?php echo $row5['cat']; ?>" class="filter">
       <span style="text-transform: uppercase;"><?php echo $row5['cat']; ?></span>
      </li>
	 <?php }
...
echo $updateQry = "INSERT INTO imagegallery VALUES ('', '$cat', '$filename', '') ";
if(mysqli_query($conn, $updateQry))
{
...
function booking($name, $email, $mob, $address,$city, $btime, $msg, $date1, $date2, $cat)
{

include ('db.php');

// $selectQry = 'SELECT * FROM booking WHERE address="'.$address.'" AND city = "'.$city.'" AND btime= "'.$btime.'" ';
// $queryresult = mysqli_query($conn, $selectQry);
...
//   if ($num_rows== 0 ){
  ...
  // } else{
		// echo "<script>alert('Booking Address City and Time Already Booked..!!');window.location='booking.php';</script>";
		// }
}
